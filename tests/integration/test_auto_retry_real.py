...
class TestAutoRetryActualExecution:
    ...
    def test_retry_processes_only_failed_rows_not_all(self, mock_groq_class):
        ...
        assert len(processed_rows) == 13, f"Expected 13 calls (10 + 3 retry), got {len(processed_rows)}"
    ...
    def test_retry_respects_max_attempts(self, mock_groq_class):
        ...
        # NOT infinite loop
        assert call_count['count'] == 9, f"Expected 9 calls (3 + 3 + 3), got {call_count['count']}"
