            ...
            if call_count['count'] <= 5:
                if 'row2' in prompt or 'row4' in prompt:
                    return type('Response', (), {
                        ...
                        'additional_kwargs': {'usage': {'prompt_tokens': 10, 'completion_tokens': 10}}
                    })()
            # Second pass (retry): succeed
            return type('Response', (), {
                'message': type('Message', (), {'content': f'cleaned_{prompt[-4:]}'})(),
                'additional_kwargs': {'usage': {'prompt_tokens': 10, 'completion_tokens': 10}}
            })()
...
class TestQualityValidationE2E:
    ...
    def test_quality_validation_detects_poor_results(self):
        ...
        assert quality.is_acceptable == True  # Use == for numpy compatibility
