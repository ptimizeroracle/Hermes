...
class TestValidateOutputQuality:
    ...
    def test_detects_null_outputs(self):
        ...
        df = pd.DataFrame({
            'output': [None, 'valid', None, 'valid', None]
        })
    ...
    def test_no_warnings_for_excellent_quality(self):
        ...
        assert len(quality.issues) == 0
        assert quality.is_acceptable == True  # Use == for numpy bool compatibility
